# Staging environment — intentionally minimal.
#
# Not instantiated here (dev-only for this demo):
#   - module.ecr           (shared ECR repos live in dev)
#   - dns-tls.tf           (would use staging.boutique.myser.serghini.me)
#   - github-oidc.tf       (account-global OIDC provider, defined in dev)
#   - karpenter-iam.tf     (would get env-scoped role names)
#   - lb-controller-*.tf   (per-cluster IRSA, defined in dev for now)
#
